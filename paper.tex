...
\texbtf{Example}. Let $L$ a lattice spanned by $v_1 = (0, 1), v_2 = (2, 0)$, and a target vector $t = (2.3, 2.4)$. The closest vector
...
\begin{itemize}
    ...
    \item If $d$ is too small (for example to accelerate decryption) then Wiener found a way to recover the private key $d$ \cite{wiener}.
...
to factorize the modulus. In 2017 the attack was used to exploit
...
of \texbtf{RSA}-$1024$ knowing the $160$ most significant bits of $p$. The attack can be expanded
until $X < p^{\frac{1}{2}}$ with a bigger lattice.

\subsection{Real World case study: ROCA}

\textbf{ROCA}\cite{roca} is a vulnerability discovered in the 2017 in a software library \textit{RSALib} in the generation
...
Where $k, a$ are random values different for every prime (unkown), $g = 65537$ and $M$ is known and it's a \textit{primorial} number.
...
Pohlig-Hellman algorithm \cite{pohlig}. With $c$ we can set a bound to the value of $a$ since
...
In practice however the bound of $a$ is too big, so the authors found a way the problem to a smaller $M'$ since the
